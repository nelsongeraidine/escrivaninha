...
Data de Atualização: 18-09-2026_Versão 0.19
...
- `src/reader/`: ReaderView, Book/Sheet/Page, controles, loading e erro, `SearchPanel.tsx` (busca no livro aberto). `Sheet.tsx` roda a virada em CSS 3D: `@keyframes` em `src/styles/book.css`, com `animationend` e fallback de 900 ms.
- `src/pdf/`: carga do documento, fila de render (concorrência 2, cancelável), cache LRU de `ImageBitmap` (24 entradas / ~100 MB), prefetch ±4 páginas. `usePageTextLayer.ts` injeta a `TextLayer` do pdf.js sobre o canvas (texto selecionável, Ctrl+F, leitura por tecnologia assistiva); reconstrói do zero a cada troca de página/zoom, sem cache. `useBookSearch.ts` indexa o texto do livro inteiro sob demanda (só na primeira busca) para achar páginas fora da tela; índice em memória, não persiste.
