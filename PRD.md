...
| 0.19 | 18-09-2026 | Busca no livro aberto (confirmado: leitor continua de 1 livro por vez, escopo não inclui biblioteca com vários PDFs). Indexação preguiçosa (só começa na primeira busca), painel com resultados (página + trecho destacado), salto direto ao clicar. Sem dependência nova. Corrigido bug real encontrado em teste: o StrictMode do React em dev deixava a indexação travada em 0% para sempre (flag de cancelamento zerada só no cleanup, não na montagem). |
